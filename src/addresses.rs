...
pub(crate) fn addresses_info(input: &str) -> IResult<&str, Vec<AddressRecord<'_>>> {
...
mod tests {
    ...
    fn test_parse_all_addresses() {
        let res = addresses_info("AUTOPUT ZA NOVI SAD: BB,284,294-296F,  BATAJNIČKI DRUM: BB,261-265,269,283-293,299,303-303A,").expect("parse the address row");
    ...
    fn test_full_row_test() {
        ...
        let res = addresses_info(TEST_INPUT);
        assert!(res.is_ok())
    }
}
